...
def get_save_path(annee, nom_tournoi):
    """Calcule le chemin de sauvegarde de manière dynamique"""
    app = App.get_running_app()
    if app:
        # Sur Android, c'est le seul dossier garanti en écriture
        base = Path(app.user_data_dir) / "save"
    else:
        base = Path.home() / "Documents" / "TournoiVercel" / "save"
    
    path = base / str(annee)
    # CRITIQUE : Créer le dossier s'il n'existe pas pour éviter OSError
