...
class VestiaireScreen(Screen):
    ...
    def fetch_convocations_from_firebase(self, data, silent=False):
        ...
        def normaliser_convocations(convocs):
            resultat = {}
            for event_id, event in convocs.items():
                if isinstance(event, dict):
                    evt = dict(event)
                    evt.pop("categorie", None)
                    resultat[event_id] = evt
                else:
                    resultat[event_id] = event
            return resultat
        ...
        def apply_result(convocations):
            ...
            if (requested_cat == self.current_cat and requested_tab == self.current_sub_tab):
                ancien = data.get("calendrier", {})
                # Comparaison en ignorant le champ "categorie"
                calendrier_modifie = (
                    normaliser_convocations(ancien)
                    !=
                    normaliser_convocations(convocations)
                )
                # Mise à jour du cache
                data["calendrier"] = convocations
                # Rafraîchissement automatique si une vraie modification est détectée
                if (
                    calendrier_modifie
                    and self.manager.current == "vestiaire"
                    and self.current_sub_tab == "CALENDRIER"
                ):
                    print("[CONVOC] Changement detecte -> Refresh calendrier")
        ...
        def do_request():
            try:
                ...
                response = requests.get(url,headers=headers,timeout=10,verify=not is_windows)
                convs = (
                    response.json()
    ...
    def on_enter(self, *args):
        ...
        Clock.schedule_once(lambda dt: self.synchroniser_role_depuis_api(),0.2)
        self._vestiaire_ready = True
        if self._pending_navigation:
            categorie, sous_onglet = self._pending_navigation
            self._pending_navigation = None
            self.current_cat = None
            Clock.schedule_once(lambda dt: setattr(self.sub_scroll, "scroll_x", 0),0.1)
            ...
            return
        if self.current_cat and self.scroll_content.children:
            # On reconstruit d'abord l'UI
            Clock.schedule_once(lambda dt: self.update_ui(), 0)
            # Puis on force le scroll à gauche après reconstruction/layout
            Clock.schedule_once(lambda dt: self.reset_sub_scroll(), 0.15)
            Clock.schedule_once(lambda dt: self.reset_sub_scroll(), 0.30)
            Clock.schedule_once(lambda dt: self.verifier_toutes_les_categories(),0.1)
            return
        self.current_cat = app.authorized_vestiaires[0]
        self.current_sub_tab = "CALENDRIER"
        Clock.schedule_once(lambda dt: self.update_ui(), 0)
        # Reset APRES reconstruction des boutons
        Clock.schedule_once(lambda dt: self.reset_sub_scroll(), 0.15)
        Clock.schedule_once(lambda dt: self.reset_sub_scroll(), 0.30)
        Clock.schedule_once(lambda dt: self.verifier_toutes_les_categories(),0.1)
        ...
        for cat in getattr(app, "authorized_vestiaires", []):
            if cat != self.current_cat and (cat_info := next((i for i in vest_cfg if i.get("categorie") == cat), None)):
                ...
                threading.Thread(
                    ...
                    args=(cat_info,path,cat,session_id),daemon=True).start()
    ...
    def update_ui(self):
        ...
        for cat in app.authorized_vestiaires:
            ...
            role = (
                app.get_role_for_cat(cat)
                if hasattr(app, "get_role_for_cat")
                else "PARENT"
            )
            disp = (
                f"{cat} "
                f"[size={int((fs + 2) * .7)}sp]"
                f"[color=888888](ADMIN)[/color][/size]"
                if role == "ADMIN"
                else cat
            )
            btn = Button(
        ...
        if data:
            ...
            cat_info = next(
                (
                    i for i in vest_cfg
                    if i.get("categorie") == self.current_cat
                ),
                None
            )
            if cat_info:
                path = os.path.join(
                    getattr(app, "user_data_dir", "."),
                    f"data_{self.current_cat}.yaml"
                )
                session_id = self._vestiaire_session_id
                target_cat = self.current_cat
                threading.Thread(target=self.verify_and_load,args=(cat_info,path,target_cat,session_id),daemon=True).start()
    
        self.check_fab_visibility()
    
    def reset_sub_scroll(self, *args):
        try:
            # Stoppe l'inertie éventuelle
            self.sub_scroll.effect_x.stop()
        except Exception:
            pass
        try:
            self.sub_scroll.effect_x.value = 0
        except Exception:
            pass
        self.sub_scroll.scroll_x = 0
        try:
            self.sub_scroll.update_from_scroll()
        except Exception:
            pass
        Clock.schedule_once(self._force_sub_scroll_left,0)
    
    def _force_sub_scroll_left(self, *args):
        try:
            self.sub_scroll.effect_x.stop()
        except Exception:
            pass
    ...
    def verify_and_load(self, cat_info, path, target_cat, session_id):
        url = (
            f"https://docs.google.com/uc?"
            f"id={cat_info.get('file_id')}&export=download"
        )
        try:
            is_windows = (platform == "win")
            r = requests.get(url,timeout=10,verify=not is_windows)
    ...
    def render_content(self, data):
        ...
        if self.current_sub_tab == "CALENDRIER":
            ...
            if not (calendrier := data.get("calendrier", {})):
                layout.add_widget(
                    InfoCard(text="Aucun événement à venir pour le moment."))
            else:
                ...
                for match_id, match_info in calendrier.items():
                    raw_date = str(match_info.get("date", "")).strip()
                    parsed_dt = None
                ...
                if evenements_a_venir:
                    contenu_a_afficher.append(("TITLE", "À venir..."))
        
                    for _, m_id, m_info in sorted(evenements_a_venir,key=lambda x: x[0]):
                        contenu_a_afficher.append(("EVENT", m_id, m_info))
        
                for mois_annee in sorted(evenements_passes.keys(),reverse=True):
                    contenu_a_afficher.append(("TITLE", mois_annee))
                    for _, m_id, m_info in sorted(evenements_passes[mois_annee],key=lambda x: x[0],reverse=True):
                        contenu_a_afficher.append(("EVENT", m_id, m_info))
        
                def stream_calendrier(items, batch_size=4):
                    total = len(items)
                    def add_batch(index, *args):
                        if self.current_sub_tab != "CALENDRIER":
                            self._populate_event = None
                            return
                        fin = min(index + batch_size, total)
                        for i in range(index, fin):
                            item = items[i]
                            if item[0] == "TITLE":
                                layout.add_widget(SectionTitle(item[1]))
                            else:
                                _, m_id, m_info = item
            ...
            if not (calendrier_saison := data.get("calendrier_saison", {})):
                layout.add_widget(EtatLabel("Aucune donnée saison trouvée."))
            else:
            ...
            if not joueurs:
                layout.add_widget(EtatLabel("Aucun joueur enregistré."))
            else:
            ...
            if not (docs := [d for d in data.get("documents", []) if d.get("nom")]):
                layout.add_widget(EtatLabel("Aucun document disponible pour le moment."))
            else:
                for doc in docs:
            ...
            if not getattr(self, "cache_membres_list", None):
                layout.add_widget(EtatLabel("Chargement des membres..."))
            ...
            if not membres_list:
                layout.add_widget(EtatLabel("Aucun membre trouvé pour cette catégorie."))
            else:
