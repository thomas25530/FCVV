...
class SoireesScreen(Screen):
    ...
    def update_scores_only(self, tournoi):
        try:
            ...
            # SÉCURISATION DE LA MISE À JOUR : On utilise nos références directes au lieu de row_layout.children
            for row_idx, widget_score in self.score_widgets.items():
                # On récupère l'index du match associé stocké lors de la création
                ...
                if match_ptr is not None and match_ptr < len(all_matchs_data):
                    ...
                    # Mise à jour des noms si dynamique
    ...
    def build_matchs_view(self, tournoi, group_color=None):
        ...
        if hasattr(self, 'matchs_header') and self.matchs_header:
            self.matchs_header.unbind(pos=self._update_rect_header, size=self._update_rect_header)
        ...
        # --- RE-CRÉATION SÉCURISÉE DU HEADER ---
                    ...
                    if classement_matches:
                        ...
                        for cm_idx, cmatch in enumerate(classement_matches):
                            ...
                            if not poules_terminees:
                                ...
                                rang = nb_eq_g - cm_idx
                                A_disp, B_disp = f"{rang}e GA", f"{rang}e GB"
                            else:
        ...
        def render_batch(dt):
            for _ in range(6):
                ...
                if isinstance(r['vals'], str) or (len(r['vals']) == 1):
                    ...
                    # Stockage des références cibles pour la mise à jour dynamique directe
                    lbl_t1, lbl_t2 = None, None
                    for i, val in enumerate(r['vals']):
                        if i >= len(column_configs): break
                        lbl = StyledLabel(
                            text=f"[b]{val}[/b]", bg_color=bg_rgb, size_hint_x=column_configs[i][1], 
                            color=((1,1,1,1) if r['force_white'] else (0,0,0,1)), 
                            halign='center', valign='middle', font_size=f_size, markup=True
                        )
                        if i == 3: lbl_t1 = lbl
                        if i == 5: lbl_t2 = lbl
                        
                        if i == 4 and r['match_idx'] is not None:
                            # Sauvegarde des métadonnées directement sur le widget de score
                            lbl._match_ptr_index = r['match_idx']
                            lbl._is_pf = r['is_pf']
                            lbl._lbl_team1 = lbl_t1
                            lbl._lbl_team2 = lbl_t2
                            self.score_widgets[r['match_idx']] = lbl
                        row.add_widget(lbl)
                container.add_widget(row)
                self._row_idx += 1
            return True

        # CORRECTION DU TIMER : Sauvegarde de la référence de la boucle temporelle pour le déchargement
